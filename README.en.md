...
- **`.vscode/` stays writable** — automatically excludes `.vscode/` via `files.readonlyExclude` so the toggle can always be flipped back and settings persist reliably.
...
- **Works without a workspace** — falls back to the user-level (global) setting when no folder is open.
...
```jsonc
{
  // written on enable (readonlyExclude keeps .vscode/ writable)
...
> Why `files.readonlyExclude`? Because `**` would also lock `.vscode/settings.json` itself, silently breaking all further writes (and making the toggle irreversible). Excluding `.vscode/` keeps the toggle working at all times — the recommended escape hatch from VS Code.
